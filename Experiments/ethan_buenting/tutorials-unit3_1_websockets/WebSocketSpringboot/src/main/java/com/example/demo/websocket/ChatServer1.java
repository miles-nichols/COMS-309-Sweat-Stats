...
public class ChatServer1 {
    ...
    public void onMessage(Session session, String message) throws IOException {
        ...
        if (message.startsWith("@")) {
            ...
            String command = message.substring(1).trim(); // Extract the command without the '/'
            switch (command.toLowerCase()) {
                case "help":
                    String helpMessage = "To send a message to all users, just enter text.\n To send a message to" +
                            " an individual, @{username} followed by text";
                    session.getBasicRemote().sendText(helpMessage);
                    break;
                default:
                    session.getBasicRemote().sendText("Unknown command: " + command);
                    break;
            }
